...
export async function chatStream({ messages, systemPrompt, onDelta, onDone, onError }) {
  const res = await fetch("/api/framegen/chat", {
    method:  "POST",
...
export async function startGeneration({ prompt, negativePrompt, model, numFrames, sceneId, imageUrl = null }) {
  const r = await fetch("/api/framegen/generate-scene", {
    method:  "POST",
    headers: { "Content-Type": "application/json" },
...
export async function startImageGeneration({ prompt, aspectRatio = "16:9" }) {
  const r = await fetch("/api/framegen/generate-image", {
    method:  "POST",
    headers: { "Content-Type": "application/json" },
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = async () => {
      try {
        ...
        const r = await fetch("/api/framegen/upload-ref", {
          method:  "POST",
          headers: { "Content-Type": "application/json" },
...
export async function fetchMediaLibrary() {
  const r = await fetch("/api/framegen/media/library");
...
export async function fetchMediaFolders() {
  const r = await fetch("/api/framegen/media/folders");
  return r.ok ? r.json() : { photos: ["Root"], videos: ["Root"] };
}

export async function createMediaFolder(type, name) {
  const r = await fetch("/api/framegen/media/folder", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function uploadMedia(type, folder, files) {
  ...
  const res = await fetch("/api/framegen/media/upload", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function listPhotos() {
  const r = await fetch("/api/framegen/photos");
...
export async function uploadPhotos(files) {
  ...
  const r = await fetch("/api/framegen/upload-photo", {
    method:  "POST",
    headers: { "Content-Type": "application/json" },
...
export async function kenBurns({ photoFile, cameraMove = "zoom-in", durationSec = 5 }) {
  const r = await fetch("/api/framegen/ken-burns", {
    method:  "POST",
    headers: { "Content-Type": "application/json" },
...
export async function stitchScenes({ sceneFiles, projectTitle }) {
  const r = await fetch("/api/framegen/stitch", {
    method:  "POST",
    headers: { "Content-Type": "application/json" },
...
export async function downloadYoutube(url, startSec, durationSec) {
  const r = await fetch("/api/framegen/youtube/download", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export const usersAPI = {
  list:   ()     => fetch("/api/framegen/users").then(r => r.json()),
  create: (name) => fetch("/api/framegen/users", { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify({ name }) }).then(r => r.json()),
};

// ─── projects ──────────────────────────────────────────────────
export const db = {
  health:  ()           => fetch("/api/framegen/health").then(r => r.json()),
  list:    (userId)     => fetch("/api/framegen/projects", { headers: { "x-user-id": userId } }).then(r => r.json()),
  save:    (userId, data) => fetch("/api/framegen/projects", { method: "POST", headers: { "Content-Type": "application/json", "x-user-id": userId }, body: JSON.stringify(data) }).then(r => r.json()),
...
export async function faceSearch({ query, sceneId, targetSec, refImagePath, onStatus, onIntent, onClip, onInfo, onDone, onError }) {
  const res = await fetch("/api/framegen/face-search", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function faceIndexList() {
  const r = await fetch("/api/framegen/face-index-list");
  return r.json();
}
...
export async function anchorPull({
  ...
  const res = await fetch("/api/framegen/anchor-pull", {
    method:  "POST",
    headers: { "Content-Type": "application/json" },
...
export async function renderWhiteboard({ scene, subject, equations, keyFacts, conceptName, visualStyle }) {
  const r = await fetch("/api/framegen/whiteboard/render", {
    method:  "POST",
    headers: { "Content-Type": "application/json" },
...
export async function fetchWhiteboardDeps() {
  const r = await fetch("/api/framegen/whiteboard/deps");
...
export async function installManim({ onLine, onDone, onError }) {
  const res = await fetch("/api/framegen/whiteboard/install-manim", { method: "POST" });
  if (!res.ok) { onError?.(`Server error ${res.status}`); return; }
